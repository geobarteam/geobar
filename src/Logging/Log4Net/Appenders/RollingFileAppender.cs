...
namespace GoSimple.Logging.Log4Net.Appenders
{
    ...
    public class RollingFileAppender : FileAppender
    {
            ...
            lock (this)
            {
                ...
                else
                {
                    if (LogLog.IsErrorEnabled)
                    {
                        ...
                        {
                            LogLog.Error(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: INTERNAL ERROR. Append is False but OutputFile [" + fileName + "] already exists.");
                        }
                    }
                }
        ...
        protected string GetOutputFileName(string fileName, int index)
        {
            return GetOutputFileName(fileName, index, true);
        }

        protected string GetOutputFileName(string fileName, int index, bool ignoreCountDirection)
        {
            FileInfo outputFile = new FileInfo(fileName);

            fileName = Path.IsPathRooted(fileName) ? outputFile.FullName : outputFile.Name;

            string name = fileName.Remove(fileName.Length - outputFile.Extension.Length, outputFile.Extension.Length);
            ...
            if (!m_staticLogFileName)
            {
                ...
                if (m_rollDate)
                {
                    fileName = name + m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "." + outputFile.Extension;
                }
                if (ignoreCountDirection || m_countDirection >= 0)
                {
                    fileName = name + '.' + index + outputFile.Extension;
                }
            }
        ...
        {
            return GetOutputFileName(fileName, m_curSizeRollBackups + 1, false);
        }
        ...
        private void DetermineCurSizeRollBackups()
        {
            ...
            LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: curSizeRollBackups starts at [" + m_curSizeRollBackups + "]");
        }
        ...
        {
            FileInfo outputFile = new FileInfo(baseFileName);

            baseFileName = Path.IsPathRooted(baseFileName) ? outputFile.FullName : outputFile.Name;

            string name = baseFileName.Remove(baseFileName.Length - outputFile.Extension.Length, outputFile.Extension.Length);

            return name + ".*" + outputFile.Extension;
        }
        ...
        private ArrayList GetExistingFiles(string baseFilePath)
        {
            ...
            using (SecurityContext.Impersonate(this))
            {
                ...
                {
                    FileInfo baseFile = new FileInfo(baseFilePath);
                    if (baseFile.Exists)
                        alFiles.Add(baseFilePath);
                    ...
                    if (files != null)
                    {
                        for (int i = 0; i < files.Length; i++)
                        {
                            ...
                            try
                            {
                                string name = curFileName.Remove(baseFile.Name.Length - (baseFile.Extension.Length + 1), baseFile.Extension.Length);

                                if (curFileName.StartsWith(name) || curFileName.EndsWith(baseFile.Extension))
                                {
                                    alFiles.Add(curFileName);
                                }
                            }
                            catch (Exception unhandeldException)
                            {
                                EventLog.WriteEntry(
                                    string.Format("Unhandeld exception while processing following file name: {0}.  The exception message was {1}.  Exception details: {2}.",
                                    curFileName, unhandeldException.Message, unhandeldException.ToString()),
                                    "logging",
            ...
            LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Searched for existing files in [" + directory + "]");
            return alFiles;
        }
        ...
        private void RollOverIfDateBoundaryCrossing()
        {
            if (m_staticLogFileName && m_rollDate)
            {
                if (FileExists(m_baseFileName))
                {
                    ...
                    LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: [" + last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "] vs. [" + m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo) + "]");
                    ...
                    if (!(last.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo).Equals(m_now.ToString(m_datePattern, System.Globalization.DateTimeFormatInfo.InvariantInfo))))
                    {
                        ...
                        LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Initial roll over to [" + m_scheduledFilename + "]");
                        RollOverTime(false);
                        LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: curSizeRollBackups after rollOver at [" + m_curSizeRollBackups + "]");
                    }
                }
            }
        }
        ...
        protected void ExistingInit()
        {
            ...
            if (AppendToFile == false)
            {
                ...
                if (fileExists)
                {
                    ...
                    {
                        LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Output file [" + fileName + "] already exists. MaxSizeRollBackups is 0; cannot roll. Overwriting existing file.");
                    }
                    else
                    {
                        LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Output file [" + fileName + "] already exists. Not appending to file. Rolling existing file out of the way.");
        ...
        private void InitializeFromOneFile(string baseFile, string curFileName)
        {
            ...
            if (m_rollDate && !m_staticLogFileName)
            {
                ...
                {
                    LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Ignoring file [" + curFileName + "] because it is from a different date period");
                    return;
                }
            }
            ...
            try
            {
                ...
                int backup;
                if (SystemInfo.TryParse(curFileName.Substring(index - 1, 1), out backup))
                {
                    if (backup > m_curSizeRollBackups)
                    {
                        ...
                        LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: File name [" + curFileName + "] moves current count to [" + m_curSizeRollBackups + "]");
                    }
                }
            }
            catch (FormatException)
            {
                ...
                LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Encountered a backup file not ending in .x [" + curFileName + "]");
            }
        }
        ...
        private RollPoint ComputeCheckPeriod(string datePattern)
        {
            ...
            for (int i = (int)RollPoint.TopOfMinute; i <= (int)RollPoint.TopOfMonth; i++)
            {
                ...
                LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Type = [" + i + "], r0 = [" + r0 + "], r1 = [" + r1 + "]");
        ...
        /// <summary>
        /// Initialize the appender based on the options set, but overwrites the
        /// Encoding property and sets it to Encoding.UTF8.
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        protected void RollOverTime(bool fileIsOpen)
        {
            if (m_staticLogFileName)
            {
                ...
                {
                    string from = GetOutputFileName(File, i);
                    string to = GetOutputFileName(m_scheduledFilename, i);
                    RollFile(from, to);
                }
        ...
        protected void RollFile(string fromFile, string toFile)
        {
            if (FileExists(fromFile))
            {
                ...
                try
                {
                    LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Moving [" + fromFile + "] -> [" + toFile + "]");
                    using (SecurityContext.Impersonate(this))
                    {
            ...
            {
                LogLog.Warn(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Cannot RollFile [" + fromFile + "] -> [" + toFile + "]. Source does not exist");
            }
        }
        ...
        protected void DeleteFile(string fileName)
        {
            if (FileExists(fileName))
            {
                ...
                {
                    LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Exception while moving file to be deleted [" + fileName + "] -> [" + tempFileName + "]", moveEx);
                }
                ...
                try
                {
                    ...
                    LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Deleted file [" + fileName + "]");
                }
                catch (Exception deleteEx)
                {
                    ...
                    else
                    {
                        ...
                        LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: Exception while deleting temp file [" + fileToDelete + "]", deleteEx);
                    }
                }
            }
        }
        ...
        protected void RollOverSize()
        {
            ...
            LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: rolling over count [" + ((CountingQuietTextWriter)QuietWriter).Count + "]");
            LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: maxSizeRollBackups [" + m_maxSizeRollBackups + "]");
            LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: curSizeRollBackups [" + m_curSizeRollBackups + "]");
            LogLog.Debug(typeof(log4net.Appender.RollingFileAppender), "RollingFileAppender: countDirection [" + m_countDirection + "]");
        ...
        protected void RollOverRenameFiles(string baseFileName)
        {
            ...
            if (m_maxSizeRollBackups != 0)
            {
                if (m_countDirection < 0)
                {
                    ...
                    {
                        DeleteFile(GetOutputFileName(baseFileName, m_maxSizeRollBackups));
                        m_curSizeRollBackups--;
                    }
                    ...
                    {
                        RollFile(GetOutputFileName(baseFileName, i), GetOutputFileName(baseFileName, i + 1));
                    }

                    m_curSizeRollBackups++;

                    // Rename fileName to fileName.1
                    RollFile(baseFileName, GetOutputFileName(baseFileName, 1));
                }
                else
                {
                    //countDirection > 0
                    if (m_curSizeRollBackups >= m_maxSizeRollBackups && m_maxSizeRollBackups > 0)
                    {
                        ...
                        DeleteFile(GetOutputFileName(baseFileName, oldestFileIndex));
                    }

                    if (m_staticLogFileName)
                    {
                        m_curSizeRollBackups++;
                        RollFile(baseFileName, GetOutputFileName(baseFileName, m_curSizeRollBackups));
                    }
                }
            }
        }
